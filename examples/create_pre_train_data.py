...
class DocumentDatabase():
    ...
    def add_document(self, document):
        ...
        # print(self.documents)documents
...
def create_instances_from_document(
    ...
    这段代码基本上是谷歌BERT的equivalent函数的复制。然而，我们做了一些改变和改进。采样得到了改进，不再需要在这个函数中进行循环。
    此外，文档的抽样与它们包含的句子的数量成比例，这意味着每个句子(而不是每个文档)有相等的机会被抽样为NextSentence任务的错误示例。"""
    ...
    # 我们*通常*想要填满整个序列，因为我们无论如何是要填充到' max_seq_length '中的，所以短序列通常是浪费的计算。
    # 然而，我们*有时*(即，short_seq_prob == 0.1 == 10 % 的时间)
    # 希望使用更短的序列来最小化预训练和微调之间的不匹配。' target_seq_length '
    # 只是一个粗略的目标，而' max_seq_length '是一个硬性限制。
    ...
    # input.
    # 我们不只是将文档中的所有标记连接到一个长序列中，
    # 然后选择任意的分割点，因为这将使下一个句子预测任务过于简单。
    # 相反，我们根据用户输入提供的实际“句子”将输入分割为“A”和“B”两个部分。
    instances = []
    current_chunk = []
    current_length = 0
    i = 0
...
def main():
    ...
    # 传入bert模型名，返回缓存了该模型的vocab.txt路径的BertTokenizer对象
    ...
    # 这里报错,输入模型名和存了该模型的vocab.txt路径的BertTokenizer对象
    ngram_dict = ZenNgramDict('./ngram/ngram.txt', tokenizer=tokenizer)
    # ngram_dict = ZenNgramDict(args.bert_name, tokenizer=tokenizer)
    print(args.reduce_memory)
# 创造DocumentDatabase类的对象docs
    with DocumentDatabase(reduce_memory=args.reduce_memory) as docs:
        print('训练语料库路径')
        print(args.train_corpus)
        print(args.reduce_memory)
        ...
        if len(docs) <= 1:
            # 错误: 在输入文件中没有发现文档中断!这些是必要的，以允许脚本确保随机NextSentences不从同一文档中取样。
            # 请添加空行以表示输入文件中的文档之间的换行。如果您的数据集不包含多个文档，则可以在任何自然边界插入空白行，例如章节、节或段落的结尾。
